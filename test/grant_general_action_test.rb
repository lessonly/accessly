...
describe Accessly do
  ...
  it "returns nil after a successful grant" do
    ...
    Accessly::Permission::Grant.new(actor).grant!(1, Post).must_be_nil
  end

  it "returns nil after a successful grant on a segment" do
    actor = User.create!

    Accessly::Permission::Grant.new(actor).on_segment(1).grant!(1, Post).must_be_nil
  end
  ...
  it "raises an error when attempting to grant" do
    ...
    proc { Accessly::Permission::Grant.new(actor).grant!(nil, Post) }.must_raise Accessly::GrantError
  end

  it "raises an error when attempting to grant a permission on an actor that is not an ActiveRecord::Base object" do
    actor = User.create!

    proc { Accessly::Permission::Grant.new(User => actor.id).grant!(1, Post) }.must_raise Accessly::GrantError
  end
end
