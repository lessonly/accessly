...
describe Accessly do
  ...
  it "returns nil after a successful grant" do
    ...
    _(Accessly::Permission::Grant.new(actor).grant!(1, Post, post.id)).must_be_nil
  end
  ...
  it "returns nil after a successful grant on a segment" do
    ...
    _(Accessly::Permission::Grant.new(actor).on_segment(1).grant!(1, Post, post.id)).must_be_nil
  end
  ...
  it "raises an error when attempting to grant" do
    ...
    _ { Accessly::Permission::Grant.new(actor).grant!(nil, Post, post.id) }.must_raise Accessly::GrantError
  end
  ...
  it "raises an error when attempting to grant a permission on an actor that is not an ActiveRecord::Base object" do
    ...
    _ { Accessly::Permission::Grant.new(User => actor.id).grant!(1, Post, post.id) }.must_raise Accessly::GrantError
  end
end
